...
const UserDeleteDialog = ({ deleteTarget, setDelete, onConfirm }) => {
 ...
 <AlertDialogTitle className="text-2xl font-semibold text-center">Terminate Entity?</AlertDialogTitle>
 <AlertDialogDescription className="text-center text-base font-medium text-muted-foreground">
 You are about to initiate a permanent registry purge for <b className="text-foreground">{deleteTarget?.name}</b>. This operation is irreversible and will sever all sub-space links.
 </AlertDialogDescription>
 </AlertDialogHeader>
 <AlertDialogFooter className="mt-10 flex sm:justify-center gap-4">
 <AlertDialogCancel className="rounded-lg h-14 px-10 border-border/20 font-semibold text-sm tracking-widest">Abort Mission</AlertDialogCancel>

...
const KanbanColumn = ({ id, title, tasks, isReadOnly, onEdit, onDelete, onStatusChange, recentlyMovedId, highlightTaskId, highlightAction, onApprove, onReject }) => {
            ...
            <ScrollArea className="flex-1 pr-1">
                <div ref={setNodeRef} className="flex flex-col gap-3 min-h-[150px] pb-4 w-full max-w-[600px] sm:max-w-none mx-auto items-center sm:items-stretch">
                    <SortableContext items={tasks.map(t => t.id)} strategy={verticalListSortingStrategy}>
                        {tasks.map((task) => (
                            <KanbanCard key={task.id} task={task} isReadOnly={isReadOnly} onEdit={onEdit} onDelete={onDelete} onStatusChange={onStatusChange} isHighlighted={task.id === recentlyMovedId || task.id === highlightTaskId} highlightAction={task.id === highlightTaskId ? highlightAction : null} onApprove={onApprove} onReject={onReject} />
                        ))}
                    </SortableContext>
                    {tasks.length === 0 && (

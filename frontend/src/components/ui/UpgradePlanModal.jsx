  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl p-0 overflow-hidden border-none bg-transparent shadow-none max-h-[95vh] Montserrat">
        <ScrollArea className="h-full max-h-[90vh] w-full rounded-[2rem] border border-white/5 bg-slate-950/98 backdrop-blur-3xl">
          <div className="relative w-full p-6 sm:p-10 overflow-hidden">
            ...
            <DialogHeader className="relative z-10 flex flex-col items-center text-center mb-10">
              ...
              <DialogTitle className="text-2xl md:text-3xl font-black text-white tracking-[0.2em] mb-3 uppercase text-center w-full">
                PLANS & PRICING
              </DialogTitle>
              <DialogDescription className="text-slate-400 text-base md:text-lg max-w-xl mx-auto font-medium leading-relaxed text-center">
              ...
              {plans.map((plan, index) => (
                <div
                  ...
                  className={cn(
                    ...
                    "bg-white/[0.02] border backdrop-blur-md",
                    plan.border,
                    plan.popular ? "ring-1 ring-primary/30 shadow-2xl shadow-primary/5 bg-white/[0.04]" : ""
                  )}
                >
                  {plan.popular && (
                  ...
                  <h3 className="text-lg font-black text-white mb-2 uppercase tracking-wider">{plan.name}</h3>
                  <p className="text-[11px] text-slate-400 mb-6 leading-relaxed font-medium min-h-[32px]">{plan.description}</p>

                  <div className="flex items-baseline gap-1 mb-8">
                    <span className="text-3xl font-black text-white tracking-tighter">{plan.price}</span>
                    {plan.price !== "Custom" && <span className="text-xs text-slate-500 font-bold">/mo</span>}
                  </div>

                  <ul className="space-y-3 mb-10 flex-1">
                    {plan.features.map((feature, i) => (
                      <li key={i} className="flex items-center gap-2.5 text-[13px] text-slate-300 font-medium">
            ...
            <div className="text-center relative z-10 pt-4">
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/5 text-slate-500 text-[10px] font-bold mb-4">
